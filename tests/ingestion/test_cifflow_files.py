"""
Integration tests for the CIF files in tests/cif_files/cifflow/.
...
from cifflow import build, ingest
...
_CIF_DIR  = pathlib.Path(__file__).parents[1] / 'cif_files' / 'cifflow'
...
def _scalar(conn, table, col, block_id):
    """Fetch one value from a structured table filtered by _cifflow_block_id."""
    row = conn.execute(
        f'SELECT {col} FROM {table} WHERE _cifflow_block_id=?', (block_id,)
    ).fetchone()
...
def _fallback(conn, block_id, tag):
    ...
    row = conn.execute(
        'SELECT value, value_type FROM _cif_fallback WHERE _cifflow_block_id=? AND tag=?',
        (block_id, tag),
    ).fetchone()
...
class TestCoreCellOnly:
    ...
    def test_stub_diffrn_created(self, cell_only_conn):
        ...
        diffrn_id = cell_only_conn.execute(
            "SELECT diffrn_id FROM cell WHERE _cifflow_block_id='test_cell_only'"
        ).fetchone()[0]
...
class TestCoreCellSu:
    ...
    def test_inline_measurand(self, cell_su_conn):
        row = cell_su_conn.execute(
            "SELECT length_a, length_c FROM cell WHERE _cifflow_block_id='test_cell_inline_su'"
        ).fetchone()
    ...
    def test_inline_su_scaled(self, cell_su_conn):
        row = cell_su_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_cell_inline_su'"
        ).fetchone()
    ...
    def test_inline_integer_su(self, cell_su_conn):
        ...
        su = cell_su_conn.execute(
            "SELECT angle_alpha_su FROM cell WHERE _cifflow_block_id='test_cell_inline_su'"
        ).fetchone()[0]
        assert su == '5'
    ...
    def test_inline_scientific_su(self, cell_su_conn):
        ...
        su = cell_su_conn.execute(
            "SELECT angle_gamma_su FROM cell WHERE _cifflow_block_id='test_cell_inline_su'"
        ).fetchone()[0]
        assert su == '0.1'
    ...
    def test_explicit_su_matches_scaled_inline(self, cell_su_conn):
        ...
        inline = cell_su_conn.execute(
            ...
            " FROM cell WHERE _cifflow_block_id='test_cell_inline_su'"
        ).fetchone()
        explicit = cell_su_conn.execute(
            "SELECT length_a_su, length_c_su, volume_su, angle_alpha_su, angle_gamma_su"
            " FROM cell WHERE _cifflow_block_id='test_cell_explicit_su'"
        ).fetchone()
...
class TestCoreAtomSiteNoAtomType:
    def test_three_atom_site_rows(self, atom_no_type_conn):
        assert atom_no_type_conn.execute(
            "SELECT COUNT(*) FROM atom_site WHERE _cifflow_block_id='test_atom_site_no_atom_type'"
        ).fetchone()[0] == 3
...
class TestCorePlaceholderInLoop:
    def test_b_iso_inapplicable(self, placeholder_conn):
        ...
        row = placeholder_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_placeholder_in_loop' AND label='Fe1'"
        ).fetchone()
        assert row[0] == '.'
    ...
    def test_occupancy_inapplicable(self, placeholder_conn):
        ...
        row = placeholder_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_placeholder_in_loop' AND label='Mn1'"
        ).fetchone()
        assert row[0] == '.'
    ...
    def test_real_value_unaffected(self, placeholder_conn):
        ...
        row = placeholder_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_placeholder_in_loop' AND label='O1'"
        ).fetchone()
        assert row == ('1.0', '0.8')
    ...
    def test_unknown_tags_in_fallback(self, unknown_tag_conn):
        tags = {
            ...
            for row in unknown_tag_conn.execute(
                "SELECT tag FROM _cif_fallback WHERE _cifflow_block_id='test_unknown_tag'"
            ).fetchall()
        }
...
class TestPowWavelengthPropagation:
    def test_three_wavelength_rows(self, wavelength_prop_conn):
        assert wavelength_prop_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_wavelength_propagation'"
        ).fetchone()[0] == 3
...
class TestPowSmallPdDataMeas:
    def test_five_pd_meas_rows(self, pd_data_meas_conn):
        assert pd_data_meas_conn.execute(
            "SELECT COUNT(*) FROM pd_meas WHERE _cifflow_block_id='test_small_pd_meas'"
        ).fetchone()[0] == 5
    ...
    def test_2theta_values(self, pd_data_meas_conn):
        vals = [
            ...
            for r in pd_data_meas_conn.execute(
                ...
                " WHERE _cifflow_block_id='test_small_pd_meas'"
                " ORDER BY _cifflow_row_id"
            ).fetchall()
        ]
...
class TestPowSmallPdMeasProc:
    def test_five_pd_meas_rows(self, pd_meas_proc_conn):
        assert pd_meas_proc_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_small_pd_meas_calc'"
        ).fetchone()[0] == 5
    ...
    def test_all_tags_in_fallback(self, fallback_scalars_conn):
        tags = {
            ...
            for r in fallback_scalars_conn.execute(
                "SELECT tag FROM _cif_fallback WHERE _cifflow_block_id='test_fallback_scalars'"
            ).fetchall()
        }
        assert tags == {
...
class TestFallbackLoop:
    def test_twelve_loop_rows(self, fallback_loop_conn):
        ...
        count = fallback_loop_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_fallback_loop' AND loop_id IS NOT NULL"
        ).fetchone()[0]
        assert count == 12
    ...
    def test_scalar_has_null_col_index(self, fallback_loop_conn):
        ...
        row = fallback_loop_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_fallback_loop' AND tag='_experiment.id'"
        ).fetchone()
    ...
    def test_loop_col_index_is_column_position(self, fallback_loop_conn):
        ...
        col_indices = {
            ...
            for r in fallback_loop_conn.execute(
                ...
                " WHERE _cifflow_block_id='test_fallback_loop' AND tag='_peak.position'"
            ).fetchall()
        }
        assert col_indices == {1}

    def test_loop_four_rows_via_cifflow_row_id(self, fallback_loop_conn):
        """Four distinct _cifflow_row_id values for _peak.position (one per loop iteration)."""
        row_ids = {
            r[0]
            for r in fallback_loop_conn.execute(
                "SELECT _cifflow_row_id FROM _cif_fallback"
                " WHERE _cifflow_block_id='test_fallback_loop' AND tag='_peak.position'"
            ).fetchall()
        }
    ...
    def test_loop_values(self, fallback_loop_conn):
        rows = fallback_loop_conn.execute(
            ...
            " WHERE _cifflow_block_id='test_fallback_loop' AND tag='_peak.intensity'"
            " ORDER BY _cifflow_row_id"
        ).fetchall()
...
class TestFallbackContainers:
    ...
    def test_list_json_content(self, fallback_containers_conn):
        from cifflow.ingestion.ingest import decode_container
        row = _fallback(fallback_containers_conn, 'test_fallback_containers',
                        '_vector.coords')
...
class TestFallbackMultiblock:
    def test_three_distinct_cifflow_block_ids(self, fallback_multiblock_conn):
        ids = {
            r[0]
            for r in fallback_multiblock_conn.execute(
                'SELECT DISTINCT _cifflow_block_id FROM _cif_fallback'
            ).fetchall()
        }
    ...
    def test_identical_content_different_blocks(self, fallback_multiblock_conn):
        """block_beta and block_gamma have identical tag values but distinct _cifflow_block_id."""
