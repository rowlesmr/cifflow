...
_FALLBACK_DDL = [
    (
        ...
        '    "col_index"   INTEGER,\n'
        '    "ref_table"   TEXT,\n'
        '    PRIMARY KEY ("_cifflow_block_id", "_cifflow_row_id", "tag")\n'
        ')'
    ),
    (
        'CREATE INDEX IF NOT EXISTS "_idx_cif_fallback_tag_block" '
        'ON "_cif_fallback" ("tag", "_cifflow_block_id")'
    ),
    (
        ...
        '    PRIMARY KEY ("_cifflow_block_id", "_audit_dataset_id")\n'
        ')'
    ),
    (
    ...
    (
        ...
        '    "_cifflow_block_id"  TEXT     PRIMARY KEY,\n'
    ...
    (
        ...
        '    PRIMARY KEY ("_cifflow_block_id", "table_name", "column_name", "pk_json")\n'
        ')'
    ),
]
...
def compactify_database(
    ...
    *dst* must be an empty, open connection.  The caller owns both connections.
    ...
    - Primary-key columns (including ``_cifflow_block_id``, ``_cifflow_row_id``,
      ``_cifflow_id``).
    - Synthetic columns (``_cifflow_block_id``, ``_cifflow_row_id``, ``_cifflow_id``).
        ...
        for tbl_name in ordered_tables:
            ...
            for col in table.columns:
                if col.name not in col_set:
                    continue
                sql_type = 'INTEGER' if col.name == '_cifflow_row_id' else 'TEXT'
            ...
            # UNIQUE(_cifflow_block_id, _cifflow_row_id) when _cifflow_row_id is not already in the PK
            if '_cifflow_row_id' in col_set and '_cifflow_row_id' not in table.primary_keys:
                col_defs.append('    UNIQUE ("_cifflow_block_id", "_cifflow_row_id")')
...
def _cast_value(
    ...
    *sql_type* is the column's SQLite affinity (``TEXT`` for non-Single
    ...
    if not isinstance(raw, str):
        return raw  # already typed (e.g. _cifflow_row_id fetched as int from source)
...
def convert_database(
    ...
    **Type mapping** (from ``ColumnDef.type_contents``):
    ...
    The synthetic ``_cifflow_row_id`` column is always ``INTEGER``; ``_cifflow_block_id`` and
    ``_cifflow_id`` are always ``TEXT``.
    ...
    **Coercion failures** (non-sentinel, non-castable values):
    ...
    try:
        ...
        for tbl_name in ordered_tables:
            ...
            # UNIQUE(_cifflow_block_id, _cifflow_row_id) when _cifflow_row_id not already in PK
            if '_cifflow_row_id' in col_name_set and '_cifflow_row_id' not in table.primary_keys:
                col_defs.append('    UNIQUE ("_cifflow_block_id", "_cifflow_row_id")')
