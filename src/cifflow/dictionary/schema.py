...
_SYNTHETIC_NAMES: frozenset[str] = frozenset({'_cifflow_block_id', '_cifflow_row_id', '_cifflow_id'})
...
def _ddl_type(col: ColumnDef) -> str:
    ...
    ``_cifflow_row_id`` is always ``INTEGER``; every other column (synthetic or domain)
    is ``TEXT``.  Domain columns store all CIF values as raw strings regardless
    of ``type_contents`` (Lesson 27).
    """
    return 'INTEGER' if col.name == '_cifflow_row_id' else 'TEXT'
