...
from cifflow.dictionary.schema import SchemaSpec, TableDef, emit_fallback_create_statements
...
def setup_duckdb(
    ...
    for tbl_name, table in schema.tables.items():
        ...
        extra = '"_cifflow_id" TEXT, ' if is_keyless else ''
        col_defs = ', '.join(f'"{c.name}" TEXT' for c in ns_cols)
        ddl = (
            f'CREATE TABLE "_raw_{tbl_name}" ('
            f'_cifflow_block_id TEXT NOT NULL, _cifflow_block_idx INTEGER NOT NULL, '
            f'_loop_id TEXT NOT NULL, _iter_idx INTEGER NOT NULL, '
            f'_cifflow_row_id INTEGER NOT NULL, '
            f'{extra}'
        )
        if col_defs:
            ddl += col_defs
...
def _load_scalar_tag(
    ...
    if location is None:
        ...
        fallback_rows.append({
            '_cifflow_block_id': block_id, '_cifflow_row_id': 1, 'tag': canonical,
...
def flush_table_batches(
    ...
    for tbl_name, rows in global_batch.items():
        ...
        all_cols = ['_cifflow_block_id', '_cifflow_block_idx', '_loop_id', '_iter_idx', '_cifflow_row_id'] + col_names
        arrow_batch = pa.record_batch({
            '_cifflow_block_id':  pa.array(bid_list,  type=pa.string()),
            '_cifflow_block_idx': pa.array(bidx_list, type=pa.int32()),
            '_loop_id':   pa.array(lid_list,  type=pa.string()),
            '_iter_idx':  pa.array(iidx_list, type=pa.int32()),
            '_cifflow_row_id':    pa.array(list(range(1, n + 1)), type=pa.int32()),
    ...
    for tbl_name in topo:
        ...
        for fk in table.foreign_keys:
            ...
            if tgt_tbl not in schema.tables:
                continue
            db.execute(f"""
                INSERT INTO "_raw_{tgt_tbl}" (_cifflow_block_id, _cifflow_block_idx, _loop_id, _iter_idx, _cifflow_row_id, "{tgt_col}")
                SELECT c._cifflow_block_id, MIN(c._cifflow_block_idx), '{_SCALARS_LOOP_ID}', 0,
                    COALESCE((SELECT MAX(_cifflow_row_id) FROM "_raw_{tgt_tbl}"), 0) + ROW_NUMBER() OVER (),
                    gen_random_uuid()::TEXT
                FROM "_raw_{tbl_name}" c
                WHERE c."{src_col}" IS NULL
                  AND NOT EXISTS (
                    SELECT 1 FROM "_raw_{tgt_tbl}" p WHERE p._cifflow_block_id = c._cifflow_block_id
                  )
                GROUP BY c._cifflow_block_id
            """)
            if populated is not None:
...
def propagate_fk_sql(
            ...
            if canonical_tbl == tbl_name:
                ...
                db.execute(f"""
                    ...
                    SET "{pk_col.name}" = (
                        ...
                        WHERE c._cifflow_block_id = s._cifflow_block_id
    ...
    for tbl_name in topo:
        ...
        for fk in table.foreign_keys:
            ...
            db.execute(f"""
                INSERT INTO "_raw_{tgt_tbl}" (_cifflow_block_id, _cifflow_block_idx, _loop_id, _iter_idx, _cifflow_row_id, {tgt_col_list})
                SELECT DISTINCT c._cifflow_block_id, c._cifflow_block_idx, '{_SCALARS_LOOP_ID}', 0,
                    COALESCE((SELECT MAX(_cifflow_row_id) FROM "_raw_{tgt_tbl}"), 0) + ROW_NUMBER() OVER (),
                    {src_col_ref}
                FROM "_raw_{tbl_name}" c
                WHERE {src_notnull}
...
def _create_final_table_ddl(tbl_name: str, table: TableDef) -> str:
    ...
    if is_keyless:
        ...
        parts.append('    "_cifflow_id" VARCHAR NOT NULL')
        parts.append('    "_cifflow_block_id"      VARCHAR NOT NULL')
        parts.append('    "_cifflow_row_id"        INTEGER NOT NULL')
        parts.append('    PRIMARY KEY ("_cifflow_id")')
    else:
        for pk in ns_pks:
            parts.append(f'    "{pk}" VARCHAR NOT NULL')
        for col in data_cols:
            parts.append(f'    "{col}" VARCHAR')
        parts.append('    "_cifflow_block_id" VARCHAR')
        parts.append('    "_cifflow_row_id"   INTEGER')
...
def _merge_keyed(
    ...
    for col in data_cols:
        if col in active_set:
            data_exprs.append(
                f'FIRST("{col}" ORDER BY _cifflow_row_id)'
                f' FILTER (WHERE "{col}" IS NOT NULL) AS "{col}"'
            )
        else:
            data_exprs.append(f'NULL AS "{col}"')
    all_insert_cols = ns_pks + data_cols + ['_cifflow_block_id', '_cifflow_row_id']
    ...
    db.execute(f"""
        ...
        SELECT
            {pk_sel},
            {data_part}FIRST(_cifflow_block_id ORDER BY _cifflow_row_id) AS _cifflow_block_id,
            MIN(_cifflow_row_id) + {row_id_offset} AS _cifflow_row_id
        FROM "_raw_{tbl_name}"
        GROUP BY {pk_sel}
        {on_conflict}
    """)
...
def _merge_keyless(
    ...
    all_insert_cols = data_cols + ['_cifflow_id', '_cifflow_block_id', '_cifflow_row_id']
    insert_col_list = ', '.join(f'"{c}"' for c in all_insert_cols)
    data_part = (', '.join(f'"{c}"' for c in data_cols) + ', ') if data_cols else ''
    db.execute(f"""
        INSERT INTO "{tbl_name}" ({insert_col_list})
        SELECT
            {data_part}gen_random_uuid()::VARCHAR AS _cifflow_id,
            _cifflow_block_id,
            _cifflow_row_id + {row_id_offset} AS _cifflow_row_id
        FROM "_raw_{tbl_name}"
...
def _populate_tag_presence(
    ...
    db.execute(f"""
        INSERT INTO "_tag_presence" ("_cifflow_block_id", "table_name", "column_name", "pk_json")
        SELECT _cifflow_block_id, '{tbl_name}', column_name, pk_json
        FROM (
            SELECT r._cifflow_block_id, json_array({pk_json_parts}) AS pk_json,
                   {select_cols}
            FROM "_raw_{tbl_name}" r
            JOIN "{tbl_name}" f USING ({pk_using})
            WHERE r._cifflow_block_id != f._cifflow_block_id
        ) t
        UNPIVOT (col_value FOR column_name IN ({unpivot_cols}))
        WHERE col_value IS NOT NULL
...
def create_final_tables(
    ...
    for tbl_name, table in schema.tables.items():
        ...
        if is_keyless:
            offset = db.execute(
                f'SELECT COALESCE(MAX(_cifflow_row_id), 0) FROM "{tbl_name}"'
            ).fetchone()[0]
