...
# ---------------------------------------------------------------------------
# _cifflow_row_id helpers
# ---------------------------------------------------------------------------

def _next_cifflow_row_id(counters: dict[str, int], table: str) -> int:
    """Claim the next _cifflow_row_id for *table* and advance the counter."""
...
def _merge_into(
    ...
    if pk not in tbl_rows:
        ...
        for col, val in row.items():
            if col in ('_cifflow_row_id', '_cifflow_block_id'):
                continue
            if val is None:
                continue
        ...
        return existing['_cifflow_row_id']
...
def _build_chain_lookups(
    ...
    *i+1*, so each lookup's output feeds into the next lookup's key.  For the
    ...
    The lookup key is the bridge table's PK value only (no ``_cifflow_block_id``
    prefix).  ``merged_rows`` is already scoped to a single dataset, so
    cross-dataset contamination cannot occur, and omitting ``_cifflow_block_id``
    ...
    for bd in bridge_columns:
        ...
        for row in merged_rows[bd.table_name].values():
            ...
            if not resolved:
                if emit_info is not None:
                    ...
                    kv: dict[str, str | None] = {
                        ...
                        '_cifflow_block_id': row.get('_cifflow_block_id'),
                    }
                    chain_details = '\n'.join(
            ...
            if len(set(resolved)) > 1 and emit is not None:
                pk_vals = {k: row.get(k) for k in ['id', '_cifflow_block_id'] if row.get(k)}
                emit(
                    f"bridge column '{bd.table_name}'.'{bd.column_name}': "
...
class _Ingester:
    def __init__(
        ...
        self._current_cifflow_block_id: str | None = None
    ...
    def _emit(self, msg: str, *, table: str | None = None, column: str | None = None, key_values: dict[str, str | None] | None = None) -> None:
        ...
        if self._on_error:
            self._on_error(msg, self._current_cifflow_block_id, severity='Warning', table=table, column=column, key_values=key_values)
    ...
    def _process_block(self, block: CifBlock) -> None:
        block_id = block.name
        self._current_cifflow_block_id = block_id
        ...
        for tbl_name, col_dict in set_buffers.items():
            ...
            row['_cifflow_block_id'] = block_id
            row['_cifflow_row_id'] = set_row_reservations[tbl_name]
            ...
            if pk not in self.merged_rows[tbl_name]:
                ...
                pk_values = dict(zip(table.primary_keys, pk))
                for col, val in row.items():
                    if col in ('_cifflow_row_id', '_cifflow_block_id') or val is None:
                        continue
                    if existing.get(col) is None:
                        existing[col] = val
                    elif existing[col] != val:
        ...
        for tbl_name, col_dict in loop_scalar_buffers.items():
            row = dict(col_dict)
            row['_cifflow_block_id'] = block_id
            row['_cifflow_row_id'] = loop_scalar_row_reservations[tbl_name]
            table = self.schema.tables[tbl_name]
            if table.primary_keys == ['_cifflow_id']:
                row['_cifflow_id'] = str(_uuid_module.uuid4())
            _apply_fk(row, table, self.schema, None,
        ...
        for tag in block.tags:
            if tag in loop_tag_to_idx:
                ...
                for iter_idx in range(n_iters):
                    row_id = _next_cifflow_row_id(self.row_id_counters, '_cif_fallback')
    ...
    def _process_scalar(
        ...
        if table.category_class == 'Set':
            # Reserve _cifflow_row_id on first scalar tag for this Set table
            if tbl_name not in set_row_reservations:
                set_row_reservations[tbl_name] = _next_cifflow_row_id(
            ...
            if tbl_name not in loop_scalar_row_reservations:
                loop_scalar_row_reservations[tbl_name] = _next_cifflow_row_id(
                    self.row_id_counters, tbl_name
                )
    ...
    def _process_loop(
                    ...
                    for tbl_name in table_names:
                        ...
                        for fk in tbl.foreign_keys:
                            ...
                            if all(v is not None for _, _, v in col_vals):
                                ...
                                if parent_table is not None:
                                    stub: dict[str, Any] = {'_cifflow_block_id': block_id}
                                    for _, tc, v in col_vals:
                                        stub[tc] = v
        ...
        # Merge all rows and collect _cifflow_row_ids for fallback
        iter_cifflow_row_ids: list[dict[str, int]] = []  # per-iteration {tbl: row_id}
        ...
        for iter_idx in range(n_iters):
            ...
            # Determine _cifflow_row_id for fallback cells
            if first_structured_table:
                fb_cifflow_row_id = iter_cifflow_row_ids[iter_idx].get(first_structured_table,
                                                        _next_cifflow_row_id(self.row_id_counters, '_cif_fallback'))
            else:
                # Pure-fallback loop
                fb_cifflow_row_id = _next_cifflow_row_id(self.row_id_counters, '_cif_fallback')
        ...
        if n_iters == 1 and table_names:
            for tbl_name in table_names:
                ...
                for k, v in row.items():
                    if v is not None and k not in ('_cifflow_block_id', '_cifflow_row_id'):
    ...
    def _flush(self) -> None:
        ...
        if self.fallback_rows:
            cur.executemany(
                ...
                [
                    (r['_cifflow_block_id'], r['_cifflow_row_id'], r['tag'], r['value'],
        ...
        if self.tag_presence_rows:
            cur.executemany(
                ...
                '("_cifflow_block_id", "table_name", "column_name", "pk_json") VALUES (?, ?, ?, ?)',
        ...
        if self.block_order_rows:
            cur.executemany(
                'INSERT OR IGNORE INTO "_block_order" ("_cifflow_block_id", "position") VALUES (?, ?)',
        ...
        if self.membership_rows:
            cur.executemany(
                ...
                '("_cifflow_block_id", "_audit_dataset_id", "id_regime") VALUES (?, ?, ?)',
                [(r['_cifflow_block_id'], r['_audit_dataset_id'], r['id_regime'])
        ...
        for row in self.membership_rows:
            if row['id_regime'] == 'assumed' and row['_audit_dataset_id'] == '':
                self.validation_rows.append({
                    ...
                    'severity': 'Warning',
                    'block_id': row['_cifflow_block_id'],
                    'detail': (
                        f"general block '{row['_cifflow_block_id']}' has non-UUID PK values "
