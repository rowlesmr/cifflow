...
def _collect_original(
    ...
    for bid in block_ids:
        ...
        fallback = _fetch_rows(conn, '_cif_fallback', '"_cifflow_block_id" = ?', (bid,))
...
def _collect_grouped(
        ...
        for pk_vals, grouped_anchor_rows in sorted(pk_groups.items()):
            ...
            for table_name in keyed_anchor_to_tables[anchor_name]:
                ...
                for bid in sorted(covered_cifflow_block_ids):
                    rows.extend(_fetch_rows(conn, table_name, '"_cifflow_block_id" = ?', (bid,)))
                if rows:
                    table_rows[table_name] = rows
            ...
            for t in block_id_tables:
                rows = []
                for bid in sorted(covered_cifflow_block_ids):
                    rows.extend(_fetch_rows(conn, t, '"_cifflow_block_id" = ?', (bid,)))
                if rows:
                    table_rows[t] = rows

            fallback: list[dict] = []
            for bid in sorted(covered_cifflow_block_ids):
                fallback.extend(_fetch_rows(conn, '_cif_fallback', '"_cifflow_block_id" = ?', (bid,)))
    ...
    for bid in remaining_cifflow_block_ids:
        ...
        for t in all_table_names:
            rows = _fetch_rows(conn, t, '"_cifflow_block_id" = ?', (bid,))
            if rows:
                table_rows[t] = rows
        fallback = _fetch_rows(conn, '_cif_fallback', '"_cifflow_block_id" = ?', (bid,))
...
def _fetch_rows_for_block(
    ...
    owned_rows: dict[tuple, dict] = {
        ...
        for row in _fetch_rows(conn, table_name, '"_cifflow_block_id" = ?', (block_id,))
    }
...
def _all_cifflow_block_ids_for_tables(conn: sqlite3.Connection, table_names: list[str]) -> list[str]:
...
def _all_cifflow_block_ids(conn: sqlite3.Connection, schema: SchemaSpec) -> list[str]:
