...
def _run_validation(
    ...
    if block_id is not None:
        rows = conn.execute(
            'SELECT DISTINCT "tag", "_cifflow_block_id" FROM "_cif_fallback" '
            'WHERE "_cifflow_block_id" = ? ORDER BY "_cifflow_block_id", "tag"',
            (block_id,),
        ).fetchall()
    else:
        rows = conn.execute(
            'SELECT DISTINCT "tag", "_cifflow_block_id" FROM "_cif_fallback" '
            'ORDER BY "_cifflow_block_id", "tag"',
        ).fetchall()

    for row in rows:
        tag = row['tag']
        bid = row['_cifflow_block_id']
        results.append(DbValidationResult(
            table='_cif_fallback',
            column='tag',
            tag=tag,
...
def _check_keyless_cardinality(
    ...
    if block_id is not None:
        rows = conn.execute(
            f'SELECT "_cifflow_block_id", COUNT(*) AS cnt FROM "{table_name}" '
            f'WHERE "_cifflow_block_id" = ? GROUP BY "_cifflow_block_id" HAVING COUNT(*) > 1',
            (block_id,),
        ).fetchall()
    else:
        rows = conn.execute(
            f'SELECT "_cifflow_block_id", COUNT(*) AS cnt FROM "{table_name}" '
            f'GROUP BY "_cifflow_block_id" HAVING COUNT(*) > 1',
        ).fetchall()
...
def _check_column(
    ...
    if pk_select:
        query = (
            f'SELECT "_cifflow_block_id", "_cifflow_row_id", {pk_select}, "{col_name}" '
            f'FROM "{table_name}"'
        )
    else:
        query = f'SELECT "_cifflow_block_id", "_cifflow_row_id", "{col_name}" FROM "{table_name}"'

    params: tuple = ()
    if block_id is not None:
        query += ' WHERE "_cifflow_block_id" = ?'
        params = (block_id,)
