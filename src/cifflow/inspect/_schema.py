...
def _load_schema(source: 'Union[str, pathlib.Path, SchemaSpec, DdlmDictionary]') -> 'SchemaSpec':
    """Resolve *source* to a :class:`~cifflow.dictionary.schema.SchemaSpec`."""
    from cifflow.dictionary.loader import DictionaryLoader, directory_resolver
    from cifflow.dictionary.schema import SchemaSpec, generate_schema
    ...
    if isinstance(source, SchemaSpec):
        return source
    if DdlmDictionary is not None and isinstance(source, DdlmDictionary):
        return generate_schema(source)
    ...
    return generate_schema(loader.load(source))
...
def _col_display_type(col: 'ColumnDef') -> str:
    """Return the display type string for a column."""
...
def _resolves_to_set(
    ...
    if target_col is not None and target_col.linked_item_id:
        return _resolves_to_set(target_col.linked_item_id, schema, tag_to_table_col, col_by_key, visited)
    return False
...
def _print_floating_loops(schema: 'SchemaSpec', *, file: TextIO) -> None:
    ...
    for table in schema.tables.values():
        ...
        has_set_link = any(
            _resolves_to_set(col.linked_item_id, schema, tag_to_table_col, col_by_key, set())
            for col in table.columns
