    ...
    for tag in block.tags:
        if tag in loop_tag_to_idx:
            ...
            for iter_idx in range(n_iters):
                row_id = _next_row_id(row_id_counters, '_cif_fallback')
...
def _flush_infra(
    ...
    if fallback_rows:
        arrow_batch = pa.record_batch({
            '_block_id':  pa.array([r['_block_id']      for r in fallback_rows], type=pa.string()),
            '_row_id':    pa.array([r['_row_id']         for r in fallback_rows], type=pa.int32()),
        ...
        db.execute(
            ...
            'SELECT "_block_id", "_row_id", "tag", "value", "value_type", "loop_id", "col_index", "ref_table" '
            'FROM __fb__ ON CONFLICT DO NOTHING'
        )
        db.unregister('__fb__')
    if block_order_rows:
        db.executemany(
            'INSERT INTO "_block_order" ("_block_id", "position") VALUES (?, ?) ON CONFLICT DO NOTHING',
            block_order_rows,
        )
    if membership_rows:
        db.executemany(
            'INSERT INTO "_block_dataset_membership" '
            '("_block_id", "_audit_dataset_id", "id_regime") VALUES (?, ?, ?) ON CONFLICT DO NOTHING',
            membership_rows,
        )
    if validation_rows:
        db.executemany(
