...
**Current state (2026-05-03, rust branch):** All 1749 tests pass. DuckDB ingest complete
(97× faster than original Python path). Phase B (PyO3 CifFile) and Phase C (DuckDB ingest)
complete. Lessons: 118.
...
1. **Resolve ONE_BLOCK fidelity mismatch classification** — 2 mismatches are intentional;
   update the fidelity check or its pass/fail criteria accordingly.

2. **Scope `OutputSpec` grouping options** — understand what flexible per-user grouping
...
5. **`CifBuilder` cross-type duplicate tag detection** — scalar-then-loop silently loses
   scalar; loop-then-scalar makes loop structurally inconsistent. Fix in `builder.py` with
   semantic errors in both cases.

6. **`source_line`/`source_col` propagation** — add to `CifBlock`, thread through
